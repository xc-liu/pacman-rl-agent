...
class DummyAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        current_action, dist = ppo_network.compute_action(current_state)

        original_dist = list(dist)

        if self.red:
            while idx_actions[current_action] not in legal_actions or idx_actions[current_action] == flip[idx_actions[current_action]]:
                del dist[dist.index(original_dist[current_action])]
                current_action = original_dist.index(max(dist))
                # return "Stop"
            return idx_actions[current_action]
        else:
            while flip[idx_actions[current_action]] not in legal_actions or idx_actions[current_action] == flip[idx_actions[current_action]]:
                del dist[dist.index(original_dist[current_action])]
                current_action = original_dist.index(max(dist))
                # return "Stop"
